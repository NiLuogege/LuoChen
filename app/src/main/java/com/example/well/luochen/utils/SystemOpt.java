...
import com.example.well.luochen.mode.data.AppSysInfo;
...
public class SystemOpt {
	...
	public void getAppInfo(){
		...
		appSysInfo.setManufacturer(Build.MANUFACTURER);
		appSysInfo.setPhoneType(Build.MODEL);
		appSysInfo.setSdkVersion(Build.VERSION.SDK);
		appSysInfo.setSystemVersion(Build.VERSION.RELEASE);
		...
		for (int i = 0; i < methods.length; i++) {
			try {
				 ...
				 String value = (String) m.invoke(appSysInfo);    //调用getter方法获取属性值
	             if(value != null){
